...
const WHATSAPP =
  `https://wa.me/${promo.whatsappNumero}?text=` +
  encodeURIComponent(promo.whatsappMensaje)
...
function Mensaje() {
  return (
    <span className="mx-6 flex items-center gap-2 text-xs sm:text-sm font-medium">
      ...
      {promo.antes} <b className="font-semibold">{promo.monto}</b> {promo.despues}
...
export default function PromoTicker() {
  ...
  return (
    <a
      ...
      aria-label={`${promo.antes} ${promo.monto} ${promo.despues}. ${promo.cta}.`}
